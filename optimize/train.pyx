...
def detect_face(photo_url, detector_backend='ssd'):
    """Detect faces in a given photo URL."""
    try:
        result = DeepFace.extract_faces(photo_url, detector_backend=detector_backend)
        embeddings = DeepFace.represent(photo_url)
        if result and result[0]['facial_area']:
            return result, embeddings
    except Exception as e:
        if detector_backend == 'ssd':
            print(f"Error: this photo does not contain a face. Trying with retinaface detector.")
            detect_face(photo_url, detector_backend='facenet')
